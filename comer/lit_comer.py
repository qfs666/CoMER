...
class LitCoMER(pl.LightningModule):
    def __init__(
        ...
        self.comer_model.load_state_dict(torch.load("/mnt/pfs_l2/jieti_team/CV/qfs/project/server_project/std_project/CoMER-master/lightning_logs/version_0/checkpoints/epoch=151-step=57151-val_ExpRate=0.6365.ckpt")['state_dict'],strict=False)
    ...
    def validation_step(self, batch: Batch, _):
        ...
        self.exprate_recorder.update([h.seq for h in hyps], batch.indices)

        # ✅ log 时使用 compute()
        self.log(
            "val_ExpRate",
            self.exprate_recorder.compute(),
            prog_bar=True,
            on_step=False,
            on_epoch=True,
        )
        print(f"val_loss = {loss} val_ExpRate = {self.exprate_recorder.compute()}")
